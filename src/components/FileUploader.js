...
const FileUploader = ({ 
  ...
  const [fileCount, setFileCount] = useState(0); // Estado para forzar re-render
  ...
  const initializeUppy = () => {
    try {
      ...
      uppyInstance.on('file-added', (file) => {
        ...
        // Actualizar contador para forzar re-render
        setFileCount(uppyInstance.getFiles().length);
        
        // Iniciar autenticación automáticamente si no está autenticado
        if (!driveAuthenticated && folderId) {
      ...
      uppyInstance.on('file-removed', (file) => {
        ...
        // Actualizar contador para forzar re-render
        setFileCount(remainingFiles.length);
      });
  ...
  const handleStartUpload = async () => {
    ...
    try {
      ...
      // Limpiar archivos de Uppy después de la subida
      uppy.getFiles().forEach(file => {
        uppy.removeFile(file.id);
      });
      
      // Resetear estados
      setHasDirectoryStructure(false);
      setFileCount(0);
  ...
  const handleAutoAuthentication = async () => {
    try {
      ...
      // Silenciar errores de auto-autenticación
    }
  };
  ...
  return (
    ...
    >
      ...
      <DialogContent>
                  ...
                  onChange={(e) => {
                    ...
                    files.forEach((file, index) => {
                      try {
                        ...
                        const fileId = `individual-${timestamp}-${index}-${random}`;
                  ...
                  onChange={(e) => {
                    ...
                    fileList.forEach(file => {
                      ...
                      if (allowedTypes.includes(fileExtension)) {
                        ...
                        // Verificar si el archivo ya existe en Uppy
                        ...
                        if (!isDuplicate) {
                          try {
                            // Generar un ID único basado en nombre, tamaño, ruta y timestamp
                            const fileId = `${file.name}-${file.size}-${file.webkitRelativePath || ''}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
                            ...
                            // Error en la adición del archivo
                          }
                        }
                      }
                    });
                    ...
                    // Resetear el input para permitir seleccionar la misma carpeta de nuevo
                    e.target.value = '';
                  }}
                  style={{ display: 'none' }}
                  id="folder-input"
        ...
        {/* Lista de archivos seleccionados - visible siempre que hay archivos */}
        {uppy && (uppy.getFiles().length > 0 || fileCount > 0) && (
      ...
      <DialogActions>
        ...
        {!uploading && !uploadResults && uppy && (uppy.getFiles().length > 0 || fileCount > 0) && (
          <Button
            variant="contained"
            startIcon={<UploadIcon />}
            onClick={handleStartUpload}
            disabled={!driveAuthenticated || !folderId || !folderInfo}
            sx={{ 
              backgroundColor: driveAuthenticated && folderId && folderInfo ? 'primary.main' : 'grey.500',
              '&:hover': {
                backgroundColor: driveAuthenticated && folderId && folderInfo ? 'primary.dark' : 'grey.600'
              }
            }}
          >
            {(() => {
              if (!folderId) return 'Sin carpeta de destino';
              if (!folderInfo) return 'Verificando carpeta...';
              if (!driveAuthenticated) return 'Conectando a Drive...';
